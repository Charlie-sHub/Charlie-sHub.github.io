...
/// Loads validated website content one section at a time.
///
/// The application layer is responsible for calling these methods separately
/// and composing the resulting app state. This interface exposes validated
/// domain entities or an [AppFailure], without surfacing validation failures
/// directly to callers.
abstract class ContentRepositoryInterface {
  ...
  /// Loads the validated project entries for the website.
  Future<Either<AppFailure, List<Project>>> loadProjects();

  /// Loads the validated security case study entries for the website.
  Future<Either<AppFailure, List<CaseStudy>>> loadCaseStudies();

  /// Loads the validated certification entries for the website.
  Future<Either<AppFailure, List<Certification>>> loadCertifications();

  /// Loads the validated course entries for the website.
